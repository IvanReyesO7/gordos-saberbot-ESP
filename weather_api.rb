...
def fetch_weather(message)
  ...
  begin
    ...
    return { mostly: '', temps: '', report: 'Preguntale a google oe :(...' }
  end
  data = JSON.parse(data_serialized)['daily'][0..3]

  days = ['hoy', 'mañana', (Date.today + 2).strftime('%A'), (Date.today + 3).strftime('%A')]
  ...
  # Report creation
  report = "Clima en #{location} para los siguientes 4 days, csm.\n"
  # If there are particular weather days
  other_weathers = weather_forcast.reject { |day| day[1] == most_freq_weather}
  report += "Exepto en #{other_weathers.map { |day| "#{day[0]}(#{day[1]})" }.join(", ")}.\n" if other_weathers.any?
  # tempreatures
  report += "\nTemperatura:\n#{weather_forcast.map { |day| " #{day[2].round}˚C para #{day[0]}" }.join("\n")}"
